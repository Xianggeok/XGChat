...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password) || empty($nickname)) {
        ...
        if ($stmt->fetch()) {
            ...
            $avatarLetter = mb_strtoupper(mb_substr($nickname, 0, 2));
            $avatarColor = randomGradient();
            $passwordHash = password_hash($password, PASSWORD_DEFAULT);

            $stmt = $db->prepare('INSERT INTO users (username, password_hash, nickname, avatar_letter, avatar_color) VALUES (?, ?, ?, ?, ?)');
            $stmt->execute([$username, $passwordHash, $nickname, $avatarLetter, $avatarColor]);
...
<head>
    ...
    <title>注册 - XGChat</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: 'Segoe UI', -apple-system, BlinkMacSystemFont, system-ui, sans-serif;
            background: #f0f2f7; min-height: 100vh;
        ...
        body::before {
            ...
            pointer-events: none; animation: bgFloat 15s ease-in-out infinite;
        }
        body::after {
            content: ''; position: fixed;
            bottom: -25%; left: -15%;
            ...
            pointer-events: none; animation: bgFloat 18s ease-in-out infinite reverse;
        }
        @keyframes bgFloat {
        ...
        .form-group input {
            ...
            font-size: 14px; background: #f8f9fc; color: #1a1d2e;
            transition: all 0.3s ease; font-family: inherit;
        }
        .form-group input:focus {
            outline: none; border-color: #667eea;
            box-shadow: 0 0 0 4px rgba(102,126,234,0.15); background: #fff;
        }
    ...
    <div class="register-container">
        ...
        <form method="POST" action="">
            <div class="form-group">
                <label>用户名</label>
                <input type="text" name="username" placeholder="3-20个字符，字母数字下划线" required autofocus value="<?php echo htmlspecialchars($_POST['username'] ?? ''); ?>">
            </div>
            <div class="form-group">
                <label>昵称</label>
                <input type="text" name="nickname" placeholder="显示在聊天中的名字" required value="<?php echo htmlspecialchars($_POST['nickname'] ?? ''); ?>">
            </div>
            <div class="form-group">
                <label>密码</label>
